...
func (g *GamesServiceV1) ListGamesDetails(uids []universes.UniverseID) ([]GameDetail, error) {
	...
	err := g.Client.Execute("GET", "games", g.Client.Path("v1/games", query), nil, &gdr)
	if err != nil {
		return nil, err
	}
...
func (g *GamesServiceV1) ListPlacesDetails(pids []PlaceID) ([]PlaceDetail, error) {
	...
	err := g.Client.Execute("GET", "games", g.Client.Path("v1/games/multiget-place-details", query), nil, &pds)
	if err != nil {
		return nil, err
	}
